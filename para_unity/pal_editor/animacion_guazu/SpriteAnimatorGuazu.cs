...
public class SpriteAnimatorGuazu : EditorWindow
{
    ...
    static readonly float altoBarraFrames = tamUnidad.y + EditorGUIUtility.singleLineHeight;
    ...
    public class PacoKeyframe
    {
        ...
        public PacoKeyframe(ObjectReferenceKeyframe keyframe, int duracion, int offset, Vector2 tamUnidad)
        {
        ...
        public PacoKeyframe(PacoKeyframe padre)
        {
    ...
    PacoKeyframe[] keyframes;
    List<PacoKeyframe> fusionados = new List<PacoKeyframe>();
    PacoKeyframe frameElegido, frameElegidoPrevio;
    ...
    void GenerarGUISKins()
    {
        ...
        if (texEliOff == null) texEliOff = new GUIContent((Texture)EditorGUIUtility.LoadRequired("Icons/iconotachooff.png"));
        if (texEliOn == null) texEliOn = new GUIContent((Texture)EditorGUIUtility.LoadRequired("Icons/iconotachoon.png"));
        if (texDupOff == null) texDupOff = new GUIContent((Texture)EditorGUIUtility.LoadRequired("Icons/iconoduplicaroff.png"));
        if (texDupOn == null) texDupOn = new GUIContent((Texture)EditorGUIUtility.LoadRequired("Icons/iconoduplicaron.png"));
    ...
    public void AplicarCambios(AnimationClip clip, PacoKeyframe[] kfs, float fps)
    {
        if (clip == null) return;
    ...
    public void CargarClip(AnimationClip nuevoClip, bool editar)
    {
        ...
        else
        {
            ...
            keyframes = new PacoKeyframe[kfs.Length];
            ...
            for (int i = 0; i < keyframes.Length; i++)
            {
                ...
                keyframes[i] = new PacoKeyframe(kfs[i], duracion, offsetAcumulado, tamUnidad);
    ...
    void MostrarBarra()
    {
            ...
            if (Event.current.rawType == EventType.MouseUp)
            {
                ...
                else if (drageando && frameElegido != null)
                {
                    if ( fusion && fusionados.Count > 0) {
                        foreach (PacoKeyframe fundir in fusionados)
                        {
                    ...
                    else if (duplicar)
                    {
                        ...
                        ArrayUtility.Insert(ref keyframes, posicionElegida, new PacoKeyframe(frameElegido));
                    }
                }
        ...
        for (int i = 0; i < keyframes.Length; i++)
        {
            if (keyframes[i].posicion.Contains(Event.current.mousePosition)) mouseOver = i;
        ...
        if (GUILayout.Button("Agregar Sprite"))
        {
            ...
            duracionTotal++;
            ArrayUtility.Add(ref keyframes, new PacoKeyframe(ork,1,0,tamUnidad));
        }
        EditorGUILayout.EndHorizontal();
